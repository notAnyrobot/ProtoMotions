...
def test_isaaclab_ws_launcher_help_is_shell_only():
    ...
    assert "Default GPU mode:    gpus" in lines
...
def test_isaaclab_ws_launcher_rejects_removed_commands(tmp_path):
...
def test_isaaclab_ws_launcher_defaults_to_shell_with_pyroki_mount(tmp_path):
    env = _base_env(tmp_path)
...
def test_isaaclab_ws_launcher_can_select_specific_normal_docker_gpu(tmp_path):
...
def test_isaaclab_ws_launcher_quotes_normal_docker_multi_gpu_request(tmp_path):
...
def test_isaaclab_ws_launcher_repairs_repo_artifact_ownership_through_container(tmp_path):
    env = _base_env(tmp_path)
    _add_pyroki_repo(env, tmp_path)
