...
#
# Shared shell-only implementation for ProtoMotions IsaacLab Docker launchers.
...
Common overrides:
  PROTO_ISAACLAB_IMAGE       Container image tag.
  ...
  PROTO_ISAACLAB_CACHE       Host IsaacLab cache root.
        ...
        manual)
            ...
            if [ "$GPU_SELECTION" != "none" ]; then
                ...
                DOCKER_GPU_ARGS+=(
                    ...
                    -e "LD_LIBRARY_PATH=$CONTAINER_PYROKI_CUDNN_LIB:/host-nvidia-libs"
                )
            fi
            ;;
        *)
...
build_runtime_args() {
    ...
    case "$NETWORK_MODE" in
        host|bridge|none)
            ...
            ;;
        *)
            fail "$NETWORK_MODE_ENV_LABEL must be host, bridge, or none"
            ;;
    esac
    ...
    case "$IPC_MODE" in
        ...
        private)
            ...
            ;;
        *)
            fail "$IPC_MODE_ENV_LABEL must be private or host"
            ;;
    esac
}
...
build_shell_args() {
    DOCKER_SHELL_ARGS=(
        ...
        -e "PYROKI_VENV=$CONTAINER_PYROKI_VENV"
        -e "PYROKI_PYTHON=$CONTAINER_PYROKI_PYTHON"
