...
#
# Convenience launcher for ProtoMotions Isaac Lab Docker on HPC-1.
#
...
# staged under /data/$USER/nvidia-libs.
...
Commands:
  ...
  reset               Run a minimal IsaacLab SimulationContext.reset() probe.
...
Environment overrides:
  ...
  PROTO_HPC_NVIDIA_LIBS       Host NVIDIA-only library dir. Default: $NVIDIA_LIBS
  PROTO_HPC_NVIDIA_SMI        Host nvidia-smi binary. Default: $NVIDIA_SMI
  ...
  PROTO_HPC_SHM_SIZE          Docker --shm-size value. Default: $SHM_SIZE
  ...
  PROTO_DOCKER_RM             1 to pass --rm, 0 to keep the container. Default: $DOCKER_RM
  PROTO_FIX_OWNERSHIP         1 to chown artifact dirs after exit, 0 to disable. Default: $FIX_OWNERSHIP
  ...
  PROTO_RESET_TIMEOUT         Timeout for reset probe in seconds. Default: $RESET_TIMEOUT
...
Examples:
  ...
  $0 nvidia-smi
  $0 smoke
  PROTO_HPC_GPUS=0 $0 reset
  PROTO_HPC_GPUS=0 PROTO_CONTAINER_NAME=proto_hpc1 PROTO_DOCKER_RM=0 $0 shell
  docker exec -it proto_hpc1 /bin/bash
  PROTO_HPC_GPUS=0,1 $0 python -c "import torch; print(torch.cuda.is_available())"
  $0 bash "/workspace/isaaclab/isaaclab.sh -p -u protomotions/train_agent.py --help"
EOF
}
...
build_gpu_ids() {
    ...
    case "$GPU_SELECTION" in
        ...
        *)
            ...
            local item
            IFS=',' read -r -a GPU_IDS <<< "$selection"
...
build_gpu_args() {
    ...
    elif [ "$GPU_SELECTION" != "all" ] && [ "${#GPU_IDS[@]}" -eq 1 ]; then
...
run_reset() {
    case "$RESET_TIMEOUT" in
        ''|*[!0-9]*)
            fail "PROTO_RESET_TIMEOUT must be a positive integer"
            ;;
    esac
