...
#
# Convenience launcher for ProtoMotions IsaacLab + PyRoki Docker on a workstation.
...
DATASET_ROOT="${PROTO_DATASET_ROOT:-/media/android/data/motion_datasets}"
CACHE_ROOT="${PROTO_ISAACLAB_CACHE:-$HOME/.cache/protomotions-isaaclab}"
...
ISAACLAB_LAUNCHER_PROFILE="workstation"
GPU_MODE_ENV_LABEL="PROTO_GPU_MODE"
GPU_SELECTION_ENV_LABEL="PROTO_GPUS"
NETWORK_MODE_ENV_LABEL="PROTO_NETWORK_MODE"
IPC_MODE_ENV_LABEL="PROTO_IPC_MODE"
GPU_MODE="${PROTO_GPU_MODE:-gpus}"
GPU_SELECTION="${PROTO_GPUS:-all}"
CUDA_VISIBLE_OVERRIDE="${PROTO_CUDA_VISIBLE_DEVICES:-}"

NVIDIA_LIBS="${PROTO_HPC_NVIDIA_LIBS:-/host-nvidia-libs}"
NVIDIA_SMI="${PROTO_HPC_NVIDIA_SMI:-/usr/bin/nvidia-smi}"
DEV_DIR="${PROTO_HPC_DEV_DIR:-/dev}"

NETWORK_MODE="${PROTO_NETWORK_MODE:-host}"
IPC_MODE="${PROTO_IPC_MODE:-host}"
SHM_SIZE="${PROTO_SHM_SIZE:-16g}"
