...
#
# Retarget a packaged AMASS SMPL MotionLib split to a robot from inside the
# IsaacLab + PyRoki HPC container.
#
# Usage:
...
the IsaacLab HPC container.

Arguments:
  split      Optional AMASS split: $SUPPORTED_SPLITS_DISPLAY (default: test)
...
Environment overrides:
  ...
  PROTO_PYTHON             Direct ProtoMotions Python interpreter. If unset,
                           uses /workspace/isaaclab/isaaclab.sh -p
  PROTO_ISAACLAB_LAUNCHER  IsaacLab launcher path.
                           Default: /workspace/isaaclab/isaaclab.sh
  PYROKI_PYTHON            PyRoki sidecar Python interpreter.
                           Default: /workspace/pyroki-venv/bin/python
...
resolve_proto_python() {
    ...
    [ -f "$python_path" ] || fail "ProtoMotions Python not found: $1"
    echo "$python_path"
}

run_proto_python() {
    if [ -n "$PROTO_PYTHON_RESOLVED" ]; then
...
is_supported_split "$SPLIT" || fail "split must be $SUPPORTED_SPLITS_DISPLAY"
...
PYROKI_PYTHON="${PYROKI_PYTHON:-/workspace/pyroki-venv/bin/python}"
PROTO_ISAACLAB_LAUNCHER="${PROTO_ISAACLAB_LAUNCHER:-/workspace/isaaclab/isaaclab.sh}"
PROTO_PYTHON_RESOLVED=""
...
[ -f "$PYROKI_PYTHON" ] || fail "PyRoki Python not found: $PYROKI_PYTHON"
[ -d "${PYROKI_REPO}/src/pyroki" ] || fail "PyRoki repo not found or invalid: $PYROKI_REPO"

if [ -n "${PROTO_PYTHON:-}" ]; then
    PROTO_PYTHON_RESOLVED="$(resolve_proto_python "$PROTO_PYTHON")"
else
...
echo "=============================================="
echo "IsaacLab HPC AMASS retargeting"
echo "=============================================="
echo "Split:               $SPLIT"
echo "Robot:               $ROBOT"
...
echo "Proto Python:        ${PROTO_PYTHON_RESOLVED:-$PROTO_ISAACLAB_LAUNCHER -p}"
